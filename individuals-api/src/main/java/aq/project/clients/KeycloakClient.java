...
public class KeycloakClient {
    ...
    private Mono<String> createUser(String adminToken, CreateUserDTO createUserDTO) {
        return webClient.post()
                ...
                .bodyValue(getUserRepresentation(createUserDTO))
                .exchangeToMono(this::extractUserId);
    }

    private UserRepresentation getUserRepresentation(CreateUserDTO createUserDTO) {
    ...
    private CredentialRepresentation getCredentialRepresentation(CreateUserDTO createUserDTO) {
    ...
    public Mono<HttpStatusCode> updateUser(UpdateUserDTO updateUserDTO) {
        return jwtClient.requestAdminToken()
                .flatMap(adminAccessToken -> webClient.put()
                        ...
                        .bodyValue(getUserRepresentation(updateUserDTO))
                        .exchangeToMono(response -> {
                            if(isErrorStatusCode(response.statusCode()))
                                return Mono.just(response.statusCode());
                            return Mono.just(HttpStatus.OK);
    ...
    private CredentialRepresentation getCredentialRepresentation(UpdateUserDTO updateUserDTO) {
        CredentialRepresentation credentialRepresentation = new CredentialRepresentation();
        credentialRepresentation.setTemporary(false);
        credentialRepresentation.setType("password");
