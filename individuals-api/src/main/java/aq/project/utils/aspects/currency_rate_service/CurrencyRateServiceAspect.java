package aq.project.utils.aspects.currency_rate_service;
...
public class CurrencyRateServiceAspect {
    ...
    @Around("execution(* aq.project.services.currency_rate.CurrencyRateService.getCurrencies(..)) && args()")
...
//        Handler logic call
    ...
    @Around("execution(* aq.project.services.currency_rate.CurrencyRateService.getCurrencyInfo(..)) && args(code)")
...
//        Prepare handler metadata
        ...
        String preMainLogicLogMessage = String.format(
                "Received request to get currency info for currency with code: %s",
                code);
        String postSuccessMainLogicCallLogMessage = String.format(
                "Success finish handle request to get currency info for currency with code: %s",
                code);
        String postFailureMainLogicCallLogMessage = String.format(
                "Error occurred during handle request to get currency info for currency with code: %s",
                code);
...
//        Handler logic call
    ...
    @Around("execution(* aq.project.services.currency_rate.CurrencyRateService.getRate(..)) && args(from, to, provider, date)")
...
//        Prepare handler metadata
        ...
        String optRateDate = Optional.ofNullable(date)
                ...
                .orElse("[not assigned]");
        String preMainLogicLogMessage = String.format(
                "Received request to get rate: [%s -> %s], provider: %s, rate date: %s",
                from, to, optProvider, optRateDate);
        String postSuccessMainLogicCallLogMessage = String.format(
                "Success handle request to get rate: [%s -> %s], provider: %s, rate date: %s",
                from, to, optProvider, optRateDate);
        String postFailureMainLogicCallLogMessage = String.format(
                "Error occurred during handle request to get rate: [%s -> %s], provider: %s, rate date: %s",
                from, to, optProvider, optRateDate);
...
//        Handler logic call
    ...
    @Around("execution(* aq.project.services.currency_rate.CurrencyRateService.getRateProviderInfo(..)) && args(code)")
...
//        Prepare handler metadata
        ...
        String tracerName = serviceName + "." + actionName + "-tracer";
        String preMainLogicLogMessage = String.format(
                "Received request to get rate provider info with code: %s",
                code);
        String postSuccessMainLogicCallLogMessage = String.format(
                "Success finish handle request to get rate provider info with code: %s",
                code);
        String postFailureMainLogicCallLogMessage = String.format(
                "Error occurred during handle request to get rate provider info with code: %s",
                code);
...
//        Handler logic call
    ...
    @Around("execution(* aq.project.services.currency_rate.CurrencyRateService.getRateProviders(..)) && args()")
