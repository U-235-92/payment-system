...
public class GetWalletInfoUnitTest {
...
//        Arrange
        ...
        Mockito.when(walletApiClient.getWalletInfo(Mockito.any(UUID.class), Mockito.any(String.class), Mockito.any(String.class)))
                .thenReturn(Mono.just(ResponseEntity.ok(walletInfoResponseDto)));

//        Act & Assert
        Assertions.assertDoesNotThrow(() -> walletService.getWalletInfo(walletId).block());

        Mockito.verify(walletApiClient, Mockito.times(1))
                .getWalletInfo(Mockito.any(UUID.class), Mockito.any(String.class), Mockito.any(String.class));
    }
}
