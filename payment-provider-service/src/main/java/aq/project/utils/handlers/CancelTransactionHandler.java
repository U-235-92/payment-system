...
public class CancelTransactionHandler {
    ...
    public void handleCancelTransaction(
            PaymentProviderServiceCancelTransactionRequestDto request
    ) {
        if(request != null) {
            UUID transactionId = request.getTransactionId();

            String traceId = request.getTraceId();
            String action = "handle-cancel-transaction-request";
            ...
            String logMessageOnReceive = String.format(
                    "Received cancel transaction request with transaction id: [%s]", transactionId);
            ...
            try(Scope scope = span.makeCurrent()) {
                ...
                TransactionStatus targetStatus = TransactionStatus.MARKED_CANCELED;
    ...
    private void validateCreateTransactionRequest(
        ...
        Set<ConstraintViolation<PaymentProviderServiceCancelTransactionRequestDto>> violationSet = validator.validate(request);

        if(!violationSet.isEmpty()) {
            Function<ConstraintViolation<PaymentProviderServiceCancelTransactionRequestDto>, String> violationToString = (violation) ->
            ...
            String logOnInvalidDto = String.format(
                    "Handle of operation [%s] was interrupted. Received invalid cancel transaction request: %s",
                    action, violations);
    ...
    private void handleOperationTransaction(
        ...
        String logMessageOnAttemptHandle = String.format(
                "Attempt to handle cancel of created transaction with transaction id: [%s]",
                transactionId);
                ...
                switch(transaction.getStatus()) {
                    ...
                    case PENDING, COMPLETED -> {
                        ...
                        String logMessageOnSuccessHandle = String.format(
                                "Attempt to handle cancel of created transaction with transaction id: [%s] completed successfully",
                                transactionId);
                    ...
                    default -> {
                        ...
                        String logMessageOnErrorHandle = String.format(
                                "Attempt to handle cancel of created transaction with transaction id: [%s] completed with error. " +
